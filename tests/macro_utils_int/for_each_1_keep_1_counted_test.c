...
#define MY_MACRO_X(N, val, var) \
    static int MU_C2(x_value_1_, var) = val; \
    static int MU_C2(x_count_, var) = N;

MU_FOR_EACH_1_KEEP_1_COUNTED(MY_MACRO_X, 5, LIST_3_ELEMENTS)

// build variables out of a list of 1 element
#define LIST_1_ELEMENT \
    41

#define MY_MACRO_Y(N, val, var) \
    static int MU_C2(y_value_1_, var) = val; \
    static int MU_C2(y_count_, var) = N;

MU_FOR_EACH_1_KEEP_1_COUNTED(MY_MACRO_Y, 5, LIST_1_ELEMENT)

int run_for_each_1_keep_1_counted_tests(void)
{
